defmodule Aesir.ZoneServer.Content.Npc.Airports.Airships.DomesticAirship do
  @moduledoc """
  Operates the timed route for the domestic airship.

  ## Behavior

  - Announces departures, travel progress, and arrivals around the route.
  - Opens and closes the exits at Juno, Einbroch, Lighthalzen, and Hugel.
  - Restarts the route timer after completing the circuit.

  ## Credits

  - Original from rAthena, authors and Contributors
    - rAthena Dev Team

  ## Adaptation

  - Transpiled by mix aesir.import.npcs from rAthena script
  - LLM-assisted Elixir refactor reviewed by Cλstor
  """

  use Aesir.ZoneServer.Npc, scope: :shared, spawn: []

  alias Aesir.ZoneServer.Script.Ctx

  @impl true
  @spec on_talk(Ctx.t()) :: Ctx.t()
  def on_talk(ctx), do: ctx

  @impl true
  @spec on_event(String.t(), Ctx.t()) :: Ctx.t()
  def on_event("OnInit", ctx) do
    initnpctimer(ctx)
  end

  def on_event("OnTimer20000", ctx) do
    mapannounce(ctx, "airplane", "We are heading to Einbroch.", 1, "0x00ff00")
  end

  def on_event("OnTimer50000", ctx) do
    mapannounce(ctx, "airplane", "We will arrive in Einbroch shortly.", 1, "0x00ff00")
  end

  def on_event("OnTimer60000", ctx) do
    ctx
    |> set_server_temp_var("airplanelocation", 1)
    |> donpcevent("#AirshipWarp-1::OnUnhide")
    |> donpcevent("#AirshipWarp-2::OnUnhide")
    |> mapannounce("airplane", "Welcome to Einbroch. Have a safe trip.", 1, "0x00ff00")
  end

  def on_event("OnTimer70000", ctx) do
    mapannounce(
      ctx,
      "airplane",
      "Currently we are in Einbroch. The Airship will take off shortly.",
      1,
      "0x00ff00"
    )
  end

  def on_event("OnTimer80000", ctx) do
    ctx
    |> donpcevent("#AirshipWarp-1::OnHide")
    |> donpcevent("#AirshipWarp-2::OnHide")
    |> mapannounce(
      "airplane",
      "The Airship is now taking off. Our next destination is Lighthalzen.",
      1,
      "0x70dbdb"
    )
  end

  def on_event("OnTimer100000", ctx) do
    mapannounce(ctx, "airplane", "We are heading to Lighthalzen.", 1, "0x70dbdb")
  end

  def on_event("OnTimer130000", ctx) do
    mapannounce(ctx, "airplane", "We will arrive in Lighthalzen shortly.", 1, "0x70dbdb")
  end

  def on_event("OnTimer140000", ctx) do
    ctx
    |> set_server_temp_var("airplanelocation", 2)
    |> donpcevent("#AirshipWarp-1::OnUnhide")
    |> donpcevent("#AirshipWarp-2::OnUnhide")
    |> mapannounce("airplane", "Welcome to Lighthalzen. Have a safe trip.", 1, "0x70dbdb")
  end

  def on_event("OnTimer150000", ctx) do
    mapannounce(
      ctx,
      "airplane",
      "Currently we are in Lighthalzen. The Airship will leave shortly.",
      1,
      "0x70dbdb"
    )
  end

  def on_event("OnTimer160000", ctx) do
    ctx
    |> donpcevent("#AirshipWarp-1::OnHide")
    |> donpcevent("#AirshipWarp-2::OnHide")
    |> mapannounce(
      "airplane",
      "The Airship is leaving the ground. Our next destination is Einbroch.",
      1,
      "0x00ff00"
    )
  end

  def on_event("OnTimer180000", ctx) do
    mapannounce(ctx, "airplane", "We are heading to Einbroch.", 1, "0x00ff00")
  end

  def on_event("OnTimer210000", ctx) do
    mapannounce(ctx, "airplane", "We will arrive in Einbroch shortly.", 1, "0x00FF00")
  end

  def on_event("OnTimer220000", ctx) do
    ctx
    |> set_server_temp_var("airplanelocation", 1)
    |> donpcevent("#AirshipWarp-1::OnUnhide")
    |> donpcevent("#AirshipWarp-2::OnUnhide")
    |> mapannounce("airplane", "Welcome to Einbroch. Have a safe trip.", 1, "0x00ff00")
  end

  def on_event("OnTimer230000", ctx) do
    mapannounce(
      ctx,
      "airplane",
      "Currently we are in Einbroch. The Airship will take off shortly.",
      1,
      "0x00ff00"
    )
  end

  def on_event("OnTimer240000", ctx) do
    ctx
    |> donpcevent("#AirshipWarp-1::OnHide")
    |> donpcevent("#AirshipWarp-2::OnHide")
    |> mapannounce(
      "airplane",
      "The Airship is now taking off. Our next destination is Juno.",
      1,
      "0xff8200"
    )
  end

  def on_event("OnTimer260000", ctx) do
    mapannounce(ctx, "airplane", "We are heading to Juno.", 1, "0xff8200")
  end

  def on_event("OnTimer290000", ctx) do
    mapannounce(ctx, "airplane", "We will arrive in Juno shortly.", 1, "0xff8200")
  end

  def on_event("OnTimer300000", ctx) do
    ctx
    |> set_server_temp_var("airplanelocation", 0)
    |> donpcevent("#AirshipWarp-1::OnUnhide")
    |> donpcevent("#AirshipWarp-2::OnUnhide")
    |> mapannounce("airplane", "Welcome to Juno. Have a safe trip.", 1, "0xff8200")
  end

  def on_event("OnTimer310000", ctx) do
    mapannounce(
      ctx,
      "airplane",
      "Currently we are in Juno. The Airship will leave shortly.",
      1,
      "0xff8200"
    )
  end

  def on_event("OnTimer320000", ctx) do
    ctx
    |> donpcevent("#AirshipWarp-1::OnHide")
    |> donpcevent("#AirshipWarp-2::OnHide")
    |> mapannounce(
      "airplane",
      "The Airship is leaving the ground. Our next destination is Hugel.",
      1,
      "0xca4bf3"
    )
  end

  def on_event("OnTimer340000", ctx) do
    mapannounce(ctx, "airplane", "We are heading to Hugel.", 1, "0xca4bf3")
  end

  def on_event("OnTimer370000", ctx) do
    mapannounce(ctx, "airplane", "We will arrive in Hugel shortly.", 1, "0xca4bf3")
  end

  def on_event("OnTimer380000", ctx) do
    ctx
    |> set_server_temp_var("airplanelocation", 3)
    |> donpcevent("#AirshipWarp-1::OnUnhide")
    |> donpcevent("#AirshipWarp-2::OnUnhide")
    |> mapannounce("airplane", "Welcome to Hugel. Have a safe trip.", 1, "0xca4bf3")
  end

  def on_event("OnTimer390000", ctx) do
    mapannounce(
      ctx,
      "airplane",
      "Currently we are in Hugel. The Airship will leave shortly.",
      1,
      "0xca4bf3"
    )
  end

  def on_event("OnTimer400000", ctx) do
    ctx
    |> donpcevent("#AirshipWarp-1::OnHide")
    |> donpcevent("#AirshipWarp-2::OnHide")
    |> mapannounce(
      "airplane",
      "The Airship is leaving the ground. Our next destination is Juno.",
      1,
      "0xff8200"
    )
  end

  def on_event("OnTimer420000", ctx) do
    mapannounce(ctx, "airplane", "We are heading to Juno.", 1, "0xff8200")
  end

  def on_event("OnTimer450000", ctx) do
    mapannounce(ctx, "airplane", "We will arrive in Juno shortly.", 1, "0xff8200")
  end

  def on_event("OnTimer460000", ctx) do
    ctx
    |> set_server_temp_var("airplanelocation", 0)
    |> donpcevent("#AirshipWarp-1::OnUnhide")
    |> donpcevent("#AirshipWarp-2::OnUnhide")
    |> mapannounce("airplane", "Welcome to Juno. Have a safe trip.", 1, "0xff8200")
  end

  def on_event("OnTimer470000", ctx) do
    mapannounce(
      ctx,
      "airplane",
      "Currently we are in Juno. The Airship will leave shortly.",
      1,
      "0xff8200"
    )
  end

  def on_event("OnTimer480000", ctx) do
    ctx
    |> donpcevent("#AirshipWarp-1::OnHide")
    |> donpcevent("#AirshipWarp-2::OnHide")
    |> mapannounce(
      "airplane",
      "The Airship is leaving the ground. Our next destination is Einbroch.",
      1,
      "0x00ff00"
    )
    |> stopnpctimer()
    |> initnpctimer()
  end
end
