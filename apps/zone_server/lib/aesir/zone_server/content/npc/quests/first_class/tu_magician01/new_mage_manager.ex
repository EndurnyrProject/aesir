defmodule Aesir.ZoneServer.Content.Npc.Quests.FirstClass.TuMagician01.NewMageManager do
  @moduledoc """
  Transpiled from rAthena quests/first_class/tu_magician01.txt:30 (New Mage Manager#M).
  ...
  use Aesir.ZoneServer.Npc,
    spawn: [
      %{
        ...
        dir: 4,
        sprite: 102,
        name: "New Mage Manager",
        unique_name: "New Mage Manager#M"
      }
    ]
  ...
  def on_talk(ctx) do
    ...
    ctx =
      if base_class(ctx) != :mage or Rathena.truthy?(:erlang.band(eaclass(ctx), 2_097_152)) do
        ctx =
          ctx
    ...
    ctx =
      if class(ctx) != :mage and get_char_var(ctx, :tu_magician01, 0) < 7 do
        ctx =
          ctx
    ...
    v6 = get_char_var(ctx, :tu_magician01, 0)
    ...
    case v6 do
      0 ->
        {ctx, v1} =
          ...
          |> next()
          |> select(["Will you teach me?", "Perhaps later..."])
        ...
        ctx =
          ...
          |> mes(
            "^ff0000Fire^000000, ^5C4033Earth^000000, ^93DB70Wind^000000, ^0000FFWater^000000."
          )
          ...
          |> set_char_var(:tu_magician01, 1)

        ctx =
          if Rathena.truthy?(checkre(ctx, 3)) do
            getexp(ctx, base_level(ctx) * 3, base_level(ctx) * 2)
          else
            getexp(ctx, base_level(ctx) * 30, base_level(ctx) * 15)
          end

        ctx = ctx |> specialeffect2(:hit5) |> close()
        throw({:script_end, ctx})
      ...
      1 ->
        ctx =
          ...
          |> mes(
            "you recover more SP while sitting down to rest. It doesn't directly affect battle, but having more SP is always better."
          )
          |> next()
          |> mes("[Mana]")
      ...
      v when v in [3, 4] ->
        ctx =
          if get_char_var(ctx, :tu_magician01, 0) == 3 do
            ctx
            |> mes("Ah, you're back?")
        ...
        ctx = ctx |> next() |> mes("[Mana]")

        ctx =
          if count_item(ctx, 915) < 10 do
            ctx =
              if get_char_var(ctx, :tu_magician01, 0) == 3 do
                ctx
                |> mes("Aw, you don't")
      ...
      27 ->
        ...
        ctx =
          ctx
          |> mes(Rathena.concat(Rathena.concat("Very nice work, ", char_name(ctx, 0)), "~"))
          |> next()
          |> mes("[Mana]")
      ...
      _ ->
        ctx =
          if get_char_var(ctx, :tu_magician01, 0) < 27 do
            ctx
            |> mes("So...")
    ...
    ctx =
      case v2 do
        ...
        3 ->
          ...
          |> mes(
            "but they'll have a greater effect than Wind or Water property skills, which would do very little or no damage at all to Wind monsters."
          )
          |> next()
          |> mes("[Mana]")
        ...
        6 ->
          {ctx, v3} =
            ...
            |> next()
            |> select(["W-wait, not yet!", "I'm ready to go on."])
            ...
            if v3 == 2 do
              ...
              ctx =
                set_local(
                  ctx,
                  :total,
                  get_local(ctx, :eread_1, 0) + get_local(ctx, :eread_2, 0) +
                    get_local(ctx, :eread_3, 0) + get_local(ctx, :eread_4, 0) +
                    get_local(ctx, :eread_5, 0)
                )

              ctx =
                if Rathena.truthy?(get_local(ctx, :total, 0)) do
                  getexp(ctx, get_local(ctx, :total, 0) * 150, get_local(ctx, :total, 0) * 70)
                else
                  ctx
                end
    ...
    ctx =
      case v4 do
        ...
        4 ->
          ...
          |> mes("a certain level.")
          |> specialeffect2(:frostdiver)
          |> next()
          |> specialeffect2(:frostdiver2)
          |> mes("[Mana]")
        ...
        8 ->
          ...
          ctx =
            set_local(
              ctx,
              :total,
              get_local(ctx, :skill_e1, 0) + get_local(ctx, :skill_e2, 0) +
                get_local(ctx, :skill_e3, 0) + get_local(ctx, :skill_e4, 0) +
                get_local(ctx, :skill_e5, 0)
            )
