defmodule Aesir.ZoneServer.Content.Npc.Cities.Prontera.GinedinRephere do
  @moduledoc """
  Discusses tailoring and the demand for dyed clothing in Prontera.

  ## Behavior

  - Shares Ginedin Rephere's views on clothing and tailoring.
  - Declines clothing dye requests because of an order backlog.
  ...
  def on_talk(ctx) do
    ...
    case choice do
      1 -> discuss_tailoring(ctx)
      2 -> decline_dye_request(ctx)
      3 -> welcome_guest(ctx)
      _ -> ctx
    end
  end

  defp discuss_tailoring(ctx) do
    ctx
    |> mes("[Tailor Ginedin Rephere]")
    |> mes(
  ...
  defp decline_dye_request(ctx) do
    ctx
    |> mes("[Tailor Ginedin Rephere]")
    |> mes("Ah...!")
    |> mes("You came to dye")
