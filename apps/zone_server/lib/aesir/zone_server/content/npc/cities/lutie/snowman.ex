defmodule Aesir.ZoneServer.Content.Npc.Cities.Lutie.Snowman do
  @moduledoc """
  Tells Snowysnow's story and rewards visitors who complete it.

  ## Behavior

  - Refuses visitors who cannot carry another item.
  - Reveals different parts of Snowysnow's history according to the `xmas_npc` story stage.
  - Advances stages 2, 6, and 10 through conversations and a salt delivery.
  - At stage 10, grants one of eight random candy, cake, or cookie rewards and sets stage 11.
  - Describes the Christmas dungeon or ends the conversation on request.

  ## Credits

  - Original from rAthena, authors and Contributors
    - rAthena Dev Team

  ## Adaptation

  - Transpiled by mix aesir.import.npcs from rAthena script
  - LLM-assisted Elixir refactor reviewed by Cλstor
  """

  use Aesir.ZoneServer.Npc,
    scope: :shared,
    spawn: [%{map: "xmas", x: 134, y: 112, dir: 4, sprite: 710, name: "Snowman", scope: :shared}]

  alias Aesir.ZoneServer.Script.Ctx

  @impl true
  @spec on_talk(Ctx.t()) :: Ctx.t()
  def on_talk(ctx) do
    if checkweight(ctx, [{1201, 1}]) == 0 do
      reject_overloaded_visitor(ctx)
    else
      start_conversation(ctx)
    end
  end

  defp reject_overloaded_visitor(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("- Wait a minute !! -")
    |> mes("- Currently you're carrying -")
    |> mes("- too many items with you. -")
    |> mes("- Please try again -")
    |> mes("- after you lose some weight. -")
    |> close()
    |> cutin("", 255)
  end

  defp start_conversation(ctx) do
    {ctx, choice} =
      ctx
      |> cutin("rutie_snownow03.bmp", 2)
      |> mes("[Snowysnow]")
      |> mes("I'm...")
      |> mes("I'm so lonely!")
      |> next()
      |> mes("[Snowysnow]")
      |> mes("Always stuck here...")
      |> mes("On the same spot...")
      |> mes(
        "Day after day after day after day after day after day after day after day after day after day..."
      )
      |> next()
      |> select([
        "^3355FFSnowysnow?^000000",
        "Info about the Christmas dungeon",
        "Quit this conversation"
      ])

    case choice do
      1 -> discuss_snowysnow(ctx)
      2 -> discuss_christmas_dungeon(ctx)
      3 -> say_goodbye(ctx)
      _ -> ctx
    end
  end

  defp discuss_snowysnow(ctx) do
    case get_char_var(ctx, :xmas_npc, 0) do
      stage when stage < 2 -> tell_origin_story(ctx)
      2 -> discuss_duffle(ctx)
      3 -> ponder_poze(ctx)
      4 -> discuss_poze(ctx)
      5 -> discuss_cantata(ctx)
      6 -> give_salt(ctx)
      7 -> remind_about_salt(ctx)
      8 -> discuss_hashokii(ctx)
      9 -> discuss_charu_charu(ctx)
      10 -> give_reward(ctx)
      11 -> greet_friend(ctx)
      _ -> discuss_christmas_dungeon(ctx)
    end
  end

  defp tell_origin_story(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "I was born in an area to the north where it snowed all the time, and was much colder than Lutie, if you can believe that."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "I was made with love by a human, and I was really happy there. Life was simple, but it was full of quiet bliss."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "I thought I could live the rest of my life in that kind of contentment. But... It was not to be."
    )
    |> next()
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "One fateful day, some ugly old woman came to our town. People say her name was 'Merlophechum,' and that she was from some strange cave town where the weather was always hot."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "On the third night she was there, she set the town on fire with frightening magic. Everyone was running in panic amongst the fear and chaos. And somehow, I was knocked out."
    )
    |> next()
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "I didn't notice how much time passed after that, but when I woke up, I was here. Well, I can say that this place, Lutie, is pretty much like heaven."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes("Everyone here is never worried, and I'm always hearing Christmas carols and stuff.")
    |> next()
    |> cutin("rutie_snownow03.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("But still...")
    |> mes(
      "Sometimes all that Christmas joy somehow doesn't cure the dark loneliness that wells deep inside of me."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes("So will you be my friend? If you do, I'll be your friend too~")
    |> close()
    |> cutin("", 255)
  end

  defp discuss_duffle(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Oh...?")
    |> mes("So you've met Duffle?")
    |> mes("Yeah, sometimes she")
    |> mes("stops by and says 'hi...'")
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "It's weird that the people of Lutie call me a mysterious, magical snowman. I mean, inside, aren't I just the same as regular people?"
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes("*Sniff* S-sometimes,")
    |> mes(
      "I don't even know what I am. But even so, the people of Lutie try to accept me no matter what."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "^3355FFPoze^000000 gave me his glamour photo with his address on it, and told me to visit him whenever I'm feeling blue. I was so happy to hear that..."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "But then fate played its cruel joke on me once again, and I realized that I had no legs to visit him with."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "How did I ever get to this town? And how in the world am I able to talk?! I-It's not natural, is it? Does... Does that make me a monster?"
    )
    |> next()
    |> mes("^3355FFSnowysnow is immersed in his deep thoughts, and seems^000000")
    |> mes("^3355FFfixated on Poze's memento.^000000")
    |> set_char_var(:xmas_npc, 3)
    |> close()
    |> cutin("", 255)
  end

  defp ponder_poze(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("...")
    |> next()
    |> mes("^3355FFSnowysnow is immersed in his deep thoughts, and seems^000000")
    |> mes("^3355FFfixated on Poze's memento.^000000")
    |> close()
    |> cutin("", 255)
  end

  defp discuss_poze(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Oh goody!")
    |> mes("You've met Poze!")
    |> next()
    |> mes("[Snowysnow]")
    |> mes("He's such an honest, good hearted guy. I hope he and Duffle get together someday.")
    |> next()
    |> mes("[Snowysnow]")
    |> mes("...Oh no~!")
    |> mes(
      "Did I say that out loud? That was supposed to stay in my head! I'm soooo sorry! Boy, I can be a real dum-dum head, huh?"
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes("Yeah...")
    |> mes("Poze is in love with Duffle. And she's kind to everybody, except for Poze.")
    |> next()
    |> mes("[Snowysnow]")
    |> mes("But I know that's because she likes him a whole whole lot! Hee hee hee!")
    |> close()
    |> cutin("", 255)
  end

  defp discuss_cantata(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Oh...?")
    |> mes("You've met")
    |> mes("^3355FFUncle Hairy Cantata^000000?")
    |> next()
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "Sure, he has a loud voice, doesn't take showers and smells like rotting food. But he's a funny guy with a warm heart. Everybody loves him!"
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "Of course, he still enjoys drinking strange things. They say it's a miracle that he doesn't have a tummyache. Hee hee~! Oh, I love that guy!"
    )
    |> close()
    |> cutin("", 255)
  end

  defp give_salt(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Oh...?")
    |> mes("You've met")
    |> mes("^3355FFAunt Thachentze^000000?")
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "Yeah, she's a pickle expert, alright. Oh right, would you give this to her? I've been keeping the roughest salt in the world for her as a bit of a favor."
    )
    |> next()
    |> set_char_var(:xmas_npc, 7)
    |> mes("^3355FFSnowysnow gave you the roughest salt in the world^000000.")
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "I like her cooking because it's soooo delicious! Sometimes, she gives me grape syrup on snow flakes. Anyway, please deliver that salt for me, buddy~!"
    )
    |> close()
    |> cutin("", 255)
  end

  defp remind_about_salt(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("'^3355FFThe roughest")
    |> mes("salt in the world^000000...'")
    |> mes(
      "Wow. Now, that's rough! Aunt Tachentze is always making pickles, so she sure could use it soon!"
    )
    |> close()
    |> cutin("", 255)
  end

  defp discuss_hashokii(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "Hashokii, the boring clown? At first, he seems kind of dumb and not really that funny. But deep inside, he cares a lot about other people."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "He's always trying his best to make those two orphans laugh and forget their troubles..."
    )
    |> close()
    |> cutin("", 255)
  end

  defp discuss_charu_charu(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Ah...")
    |> mes(
      "So you've met Charu Charu? That boy is so full of optimism and always looking forward. When he grows up, he's going to be a big shot!"
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes("I'm sure of it!")
    |> mes("Hee hee hee~!")
    |> close()
    |> cutin("", 255)
  end

  defp give_reward(ctx) do
    ctx =
      ctx
      |> cutin("rutie_snownow01.bmp", 2)
      |> mes("[Snowysnow]")
      |> mes(
        "Thank you for listening to me so far. I really appreciate that you try to understand me, even though you're a stranger here."
      )
      |> next()
      |> mes("[Snowysnow]")
      |> mes(
        "Now, you know me better than anyone else in this town. So, in return, I want to give you a small present."
      )
      |> next()
      |> mes("[Snowysnow]")
      |> mes("Tah dah!")
      |> mes("Pick anything")
      |> mes("you want in here~")
      |> next()
      |> mes("^3355FFYou gingerly stir")
      |> mes("your hand around in")
      |> mes("Snowysnow's magical gift bag^000000.")
      |> next()

    ctx
    |> grant_reward(Enum.random(1..8))
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "My dear friend, please visit me from time to time, so that we can chitchat, okay? See you soon! And Merry Christmas!"
    )
    |> close()
    |> cutin("", 255)
  end

  defp grant_reward(ctx, 1) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(529, 5)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Wow~!")
    |> mes("^3355FF5 Candy^000000!")
    |> mes("Congratulations!")
  end

  defp grant_reward(ctx, 2) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(529, 10)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Ooh~!")
    |> mes("^3355FF10 Candy^000000!")
  end

  defp grant_reward(ctx, 3) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(530, 5)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Hoooraaaay~!")
    |> mes("^3355FF5 Candy Cane^000000!")
  end

  defp grant_reward(ctx, 4) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(530, 10)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Wow, that's so great!")
    |> mes("^3355FF10 Candy Cane^000000!")
  end

  defp grant_reward(ctx, 5) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(539, 1)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Aren't you lucky!")
    |> mes("^3355FF1 Piece Of Cake^000000!")
  end

  defp grant_reward(ctx, 6) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(539, 2)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Now, isn't that nice?")
    |> mes("^3355FF2 Piece Of Cake^000000!")
  end

  defp grant_reward(ctx, 7) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(538, 5)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Oh woooooow~!")
    |> mes("^3355FF5 Cookie^000000!")
  end

  defp grant_reward(ctx, 8) do
    ctx
    |> set_char_var(:xmas_npc, 11)
    |> give_item(538, 10)
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("*Gasp!* Ooh~")
    |> mes("^3355FF10 Cookie^000000!")
  end

  defp greet_friend(ctx) do
    ctx
    |> cutin("rutie_snownow02.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Hello hello!")
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "You're always welcome in Lutie, especially by me, Snowysnow! Happy Kwanza, Happy Hannukah and Merry Christmas~!"
    )
    |> close()
    |> cutin("", 255)
  end

  defp discuss_christmas_dungeon(ctx) do
    ctx
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "Around this wonderful town, eternally blessed with Christmas, there is a horrible dungeon, eternally cursed with Christmas."
    )
    |> next()
    |> mes("[Snowysnow]")
    |> mes(
      "I've heard that it's well decorated and looks just like a Toy Factory inside, where everything is so cute and pretty. They are Toy Soldiers and Gift Boxes as far as the eye can see!"
    )
    |> next()
    |> cutin("rutie_snownow01.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes(
      "Isn't that soooo exciting?! *Sigh* Even if it is a dungeon, I would like to go inside just to look. If only I was a real boy, or even had legs..."
    )
    |> close()
    |> cutin("", 255)
  end

  defp say_goodbye(ctx) do
    ctx
    |> cutin("rutie_snownow03.bmp", 2)
    |> mes("[Snowysnow]")
    |> mes("Bye bye, friend~!")
    |> mes("Thank you for listening to me~")
    |> mes("I'll see you again, someday! You'll always be in my heart~")
    |> close()
    |> cutin("", 255)
  end
end
