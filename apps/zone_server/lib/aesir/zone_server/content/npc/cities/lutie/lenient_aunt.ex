defmodule Aesir.ZoneServer.Content.Npc.Cities.Lutie.LenientAunt do
  @moduledoc """
  Advances the Snowysnow story through Thachentze's salt delivery.

  ## Behavior

  - At stage 5, requests Snowysnow's roughest salt and advances the story to stage 6.
  - At stage 6, reminds the visitor to fetch the salt.
  - At stage 7, accepts the salt, explains Snowysnow's gift bag, and advances the story to stage 8.
  ...
  use Aesir.ZoneServer.Npc,
    scope: :shared,
    spawn: [
      %{map: "xmas_in", x: 27, y: 103, dir: 4, sprite: 701, name: "Lenient Aunt", scope: :shared}
    ]
  ...
  def on_talk(ctx) do
    case get_char_var(ctx, :xmas_npc, 0) do
      5 -> request_salt(ctx)
      6 -> remind_about_salt(ctx)
      7 -> accept_salt(ctx)
      _ -> greet_visitor(ctx)
    end
  end

  defp request_salt(ctx) do
    ctx
    |> mes("[Thachentze]")
