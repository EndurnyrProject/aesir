defmodule Aesir.ZoneServer.Content.Npc.Cities.Umbala.Bertan do
  @moduledoc """
  Tells a random joke once the visitor understands Umbalan speech.

  ## Behavior

  - Speaks unintelligibly before Umbalan language progress reaches stage 3.
  - Otherwise chooses one of six jokes at random.
  ...
  defp tell_random_joke(ctx) do
    case Enum.random(1..6) do
      1 -> pig_joke(ctx)
      2 -> orc_lady_joke(ctx)
      3 -> isis_joke(ctx)
      4 -> abandoned_joke(ctx)
      5 -> gm_joke(ctx)
      6 -> bar_joke(ctx)
      _ -> close(ctx)
    end
