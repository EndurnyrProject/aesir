defmodule Aesir.ZoneServer.Content.Npc.Cities.Umbala.Haith do
  @moduledoc """
  Sells La Ed and Haith's rumor about Niels's treasure for 1,000 zeny.

  ## Behavior

  - Charges 1,000 zeny before revealing that Niels is in the village's northern area.
  - Refuses to share the rumor when the visitor declines or cannot afford it.
  ...
  def on_talk(ctx) do
    ...
    case choice do
      1 -> buy_rumor(ctx)
      _ -> refuse_payment(ctx)
    end
  end

  defp buy_rumor(ctx) do
    if zeny(ctx) > 999 do
      reveal_rumor(ctx)
    else
      ctx
