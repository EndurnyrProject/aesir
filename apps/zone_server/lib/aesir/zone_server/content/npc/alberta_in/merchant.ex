defmodule Aesir.ZoneServer.Content.Npc.AlbertaIn.Merchant do
  @moduledoc """
  ...
  use Aesir.ZoneServer.Npc,
    spawn: [%{map: "alberta_in", x: 53, y: 43, dir: 6, sprite: 86, name: "Merchant"}]
    ...
    ctx =
      if get_char_var(ctx, :Upper, 0) == 1 do
        if class(ctx) == get_char_var(ctx, :Job_Novice_High, 0) and
             (get_char_var(ctx, :ADVJOB, 0) == get_char_var(ctx, :Job_Whitesmith, 0) or
                get_char_var(ctx, :ADVJOB, 0) == get_char_var(ctx, :Job_Creator, 0)) do
          ctx =
            ctx
            |> mes("[Chief Mahnsoo]")
          ...
          ctx = jobchange(ctx, get_char_var(ctx, :Job_Merchant_High, 0))
      ...
      case v2 do
        1 ->
          ...
          if get_char_var(ctx, :BaseJob, 0) != :novice do
            ctx =
              if get_char_var(ctx, :BaseJob, 0) == :merchant do
                ctx
