defmodule Aesir.ZoneServer.Content.Npc.Jobs.M22.Rogue.MobRogue101264 do
  @moduledoc """
  Springs a monster ambush on Rogue candidates in the job test tunnel.

  ## Behavior

  - Summons two Abysmal Knights when a Thief steps into its trigger area.
  ...
  @mob_dead_event "mob_rogue#7::OnMyMobDead"
  ...
  def on_event("OnTouch", ctx) do
    if Rathena.job_id(base_job(ctx)) == Rathena.job_id(:thief) do
      ctx
      |> summon_mob(mob_id: 1219, map: "in_rogue", at: {101, 264}, event: @mob_dead_event)
      |> summon_mob(mob_id: 1219, map: "in_rogue", at: {102, 264}, event: @mob_dead_event)
    else
