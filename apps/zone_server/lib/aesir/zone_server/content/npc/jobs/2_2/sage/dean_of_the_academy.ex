defmodule Aesir.ZoneServer.Content.Npc.Jobs.M22.Sage.DeanOfTheAcademy do
  @moduledoc """
  Kayron Grik, dean of the Schweicherbil Magic Academy, who evaluates dissertations and
  promotes Mages to Sages.

  ## Behavior

  - Turns away transcendent characters and characters who are not Mages.
  - Explains the Sage path to Mages who have not started the quest.
  - Accepts the finished dissertation from qualified Mages (job level 40, no unspent skill
    points), completes the quest, and changes the character into a Sage.
  - Resets the quest when an unqualified Mage presents a dissertation.
  ...
  def on_talk(ctx) do
    ...
    cond do
      upper(ctx) == 1 ->
        greet_transcendent(ctx)

      Rathena.job_id(base_job(ctx)) != Rathena.job_id(:mage) ->
        ctx |> greet_non_mage() |> close() |> cutin("job_sage_kayron", 255)

      get_char_var(ctx, :SAGE_Q, 0) == 0 ->
        introduce_sage_path(ctx)

      get_char_var(ctx, :SAGE_Q, 0) == 15 and count_item(ctx, 1550) > 0 ->
        review_dissertation(ctx)

      get_char_var(ctx, :SAGE_Q, 0) == 15 ->
        ctx
  ...
  defp introduce_sage_path(ctx) do
    {ctx, choice} =
      ctx
    ...
    ctx
    |> answer_introduction(choice)
    |> close()
    |> cutin("job_sage_kayron", 255)
  end

  defp answer_introduction(ctx, 1) do
    ctx
    |> mes("[Kayron Grik]")
  ...
  defp answer_introduction(ctx, 2) do
    ctx
    |> mes("[Kayron Grik]")
  ...
  defp answer_introduction(ctx, 3) do
    ctx
    |> mes("[Kayron Grik]")
    |> mes(
  ...
  defp answer_introduction(ctx, _choice), do: ctx

  defp review_dissertation(ctx) do
    cond do
      job_level(ctx) < 40 ->
        ctx
