defmodule Aesir.ZoneServer.Content.Npc.Jobs.M11e.Ninja.KuugaGai do
  @moduledoc """
  Kuuga Gai, the High Ninja who runs the Ninja job quest.

  ## Behavior

  - Sends Novices of Job Level 10 or higher to deliver a truce letter to Wildcat Joe.
  - Asks for 5 Cyfars and 1 Phracon once Joe's reply is read.
  - Changes applicants with no unused skill points to Ninja, taking the materials and
    giving a weapon.
  - Turns away baby classes and greets Ninjas and other classes.
  ...
  def on_talk(ctx) do
    cond do
      ...
      Rathena.job_id(base_class(ctx)) == Rathena.job_id(:ninja) ->
        greet_ninja(ctx)
  ...
  defp talk_to_novice(ctx) do
    if job_level(ctx) < 10 do
      ...
      quest_progress(ctx, get_char_var(ctx, :NINJ_Q, 0))
    end
  end

  defp quest_progress(ctx, 0), do: offer_quest(ctx)
  ...
  defp quest_progress(ctx, 3), do: read_reply(ctx)

  defp quest_progress(ctx, 4) do
    cond do
      count_item(ctx, 7053) < 5 or count_item(ctx, 1010) < 1 ->
        ctx
        |> mes("[Kuuga Gai]")
        |> mes("Hurry and bring")
  ...
  defp quest_progress(ctx, _step), do: greet_ninja(ctx)

  defp offer_quest(ctx) do
