defmodule Aesir.ZoneServer.Content.Npc.Jobs.M22.Sage.HistoryProfessor do
  @moduledoc """
  Saphien Layless, the Sage academy history professor who teaches Yggdrasil and guides the
  candidate's thesis.

  ## Behavior

  - Greets non-Mages with class-specific small talk.
  - Quizzes a newly assigned candidate about Yggdrasil and asks for Yggdrasil items.
  - Lectures on Yggdrasil once the candidate brings a Yggdrasilberry, Seed, or Leaf, then
    asks for thesis materials.
  - Consumes the materials and runs a thesis quiz; any wrong answer fails the attempt, while
    a flawless thesis grants the dissertation and advances the quest.
  ...
  defp talk_to_candidate(ctx, 10) do
  ...
  defp introduce_class(ctx) do
    {ctx, choice} =
      ctx
      |> mes("Welcome, #{char_name(ctx, 0)}. I am glad to meet you.")
    ...
    ctx
    |> answer_yggdrasil_familiarity(choice)
    |> mes("[Saphien Layless]")
  ...
  defp answer_yggdrasil_familiarity(ctx, 1) do
    {ctx, answer} =
      ctx
      |> mes("[Saphien Layless]")
    ...
    ctx
    |> answer_what_is_yggdrasil(answer)
    |> next()
  end

  defp answer_yggdrasil_familiarity(ctx, 2) do
    ctx
    |> mes("[Saphien Layless]")
  ...
  defp answer_yggdrasil_familiarity(ctx, _choice), do: ctx

  defp answer_what_is_yggdrasil(ctx, 1) do
    ctx
    |> mes("[Saphien Layless]")
    |> mes("Wrong. #{char_name(ctx, 0)}, you got - 10 points.")
  ...
  defp answer_what_is_yggdrasil(ctx, 2) do
    ctx
    |> mes("[Saphien Layless]")
  ...
  defp answer_what_is_yggdrasil(ctx, 3) do
    ctx
    |> mes("[Saphien Layless]")
    |> mes("Great Schott...If you don't know, just say so.")
    |> mes(
      "By the way, #{char_name(ctx, 0)}, you just got - 10 points for lying and for being wrong."
    )
  end

  defp answer_what_is_yggdrasil(ctx, _answer), do: ctx

  defp hold_class(ctx) do
    cond do
      count_item(ctx, 607) > 0 ->
        ctx
        |> mes("[Saphien Layless]")
        ...
        |> next()
        |> lecture_on_yggdrasil()

      count_item(ctx, 608) > 0 ->
        ctx
        |> mes("[Saphien Layless]")
      ...
      count_item(ctx, 610) > 0 ->
        ...
        |> mes("So, I must give you - 10 points, #{char_name(ctx, 0)}.")
        |> next()
        |> mes("[Saphien Layless]")
        |> mes("Okay, now I am starting the class.")
        |> mes(
  ...
  defp lecture_on_yggdrasil(ctx) do
    ctx
    |> mes("[Saphien Layless]")
  ...
  defp write_thesis(ctx) do
    ...
    if wrong > 0 do
      ...
      write_thesis_conclusion(ctx, wrong)
    end
  end

  defp write_thesis_conclusion(ctx, wrong) do
    {ctx, wrong} =
      ctx
  ...
  defp thesis_line(ctx, wrong, correct, options, echoes) do
    {ctx, choice} = select(ctx, options)

    cond do
      choice not in 1..length(echoes) ->
        {ctx, wrong}

      choice == correct ->
        {mes(ctx, Enum.at(echoes, choice - 1)), wrong}

      true ->
        {mes(ctx, Enum.at(echoes, choice - 1)), count_wrong_answer(ctx, wrong)}
    end
  end

  defp count_wrong_answer(%Ctx{status: {:error, _}}, wrong), do: wrong
  defp count_wrong_answer(_ctx, wrong), do: wrong + 1

  defp reject_thesis(ctx) do
    ctx
    |> mes("[Saphien Layless]")
    |> mes("...#{char_name(ctx, 0)}, have you lost your mind?")
