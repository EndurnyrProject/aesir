defmodule Aesir.ZoneServer.Content.Npc.Jobs.M22.Alchemist.StudyingMan do
  @moduledoc """
  Darwin, a heartbroken researcher who teaches Alchemist candidates to make simple medicine.

  ## Behavior

  - Refuses to talk while the player is carrying too much.
  - Laments his lost love Harmona to non-Merchants and congratulates new Alchemists.
  - Asks candidates for Medicine Bowls, Empty Bottles, and Red, Yellow, and White Herbs,
    advancing the quest log.
  - With the materials, walks the candidate through a six-step brewing test; any wrong step
    ruins the mixture, while a perfect run rewards the potions and sends them to Van Helmont.
  - Trades his whole knowledge for an Illusion Flower, readying the candidate for the Union
    Leader directly.
  ...
  # {options, %{wrong choice => Darwin's reaction lines}}
  @brewing_steps [
    {[
    ...
    {[
     ...
     %{
       1 => ["...Eh!?", "That's not", "medicine!"],
       3 => ["A Harp?", "And how would", "you do that?"]
     }},
    {["Crush the Herbs.", "Crush the Medicine Bowl.", "Crush Darwin's foot."],
     %{
       2 => ["Wh-What are", "you doing!?"],
       3 => ["Agh...!", "What do you", "think you're", "doing?!"]
     }},
    {["Spray clean water.", "Drink clean water.", "Pour clean water."],
     %{
       1 => ["Huh?", "What are you doing?"],
       2 => ["W-wait...", "Are you", "taking a break?"]
     }},
    {[
     ...
     %{
       2 => ["Eat the Herbs?", "I think you need", "to focus on the", "task at hand..."],
       3 => ["Singing and", "dancing? Alchemists", "don't do that, have", "you gone crazy?"]
     }},
    {[
     ...
     %{
       1 => ["We're Alchemists,", "not restaurant chefs."],
       3 => ["Huh...", "Pretty sloppy..."]
     }}
  ]
  ...
  defp talk(ctx) do
    cond do
      Rathena.job_id(base_job(ctx)) != Rathena.job_id(:merchant) and
      ...
      get_char_var(ctx, :ALCH_Q, 0) == 6 ->
        first_meeting(ctx)

      get_char_var(ctx, :ALCH_Q, 0) == 7 ->
        materials_check(ctx)

      get_char_var(ctx, :ALCH_Q, 0) == 8 ->
        ctx
        |> mes("I said to go")
        |> mes("to Van Helmont.")
  ...
  defp first_meeting(ctx) do
    ...
    first_meeting_choice(choice, ctx)
  end

  defp first_meeting_choice(1, ctx) do
    ctx
    |> mes("[Darwin]")
    |> mes("You wish to")
    |> mes("learn Alchemy?")
  ...
  defp first_meeting_choice(2, ctx) do
    ctx
    |> mes("[Darwin]")
    |> mes("Flowers...?")
    |> mes("In the darkest")
  ...
  defp first_meeting_choice(3, ctx) do
    ctx
    |> mes("[Darwin]")
    |> mes("Consider what")
  ...
  defp first_meeting_choice(_, ctx), do: ctx

  defp materials_check(ctx) do
    ctx =
      ctx
      |> mes("...")
      |> mes("......")
    ...
    cond do
      count_item(ctx, 710) > 0 ->
        illusion_flower(ctx)

      count_item(ctx, 7134) > 2 and count_item(ctx, 713) > 2 and count_item(ctx, 507) > 0 and
        count_item(ctx, 508) > 0 and count_item(ctx, 509) > 0 ->
        brewing_lesson(ctx)
  ...
  defp brewing_lesson(ctx) do
    ...
    {ctx, wrong_steps} = Enum.reduce(@brewing_steps, {ctx, 0}, &brewing_step/2)

    # A halted ctx never recorded the mistakes (set_local is a no-op), so it read 0.
    wrong_steps = if match?({:error, _}, ctx.status), do: 0, else: wrong_steps
  ...
  defp brewing_step({options, reactions}, {ctx, wrong_steps}) do
    {ctx, choice} = select(ctx, options)

    case Map.fetch(reactions, choice) do
      {:ok, lines} ->
        ctx = Enum.reduce(["[Darwin]" | lines], ctx, &mes(&2, &1))
        {next(ctx), wrong_steps + 1}
