defmodule Aesir.ZoneServer.Content.Npc.Jobs.M21a.Whitesmith.Mastersmith do
  @moduledoc """
  Advances eligible High Merchants to MasterSmith (Whitesmith) in Valhalla.

  ## Behavior

  - Players who are not transcendent or have no pending advancement receive a random
    congratulation or thoughts on the pinnacle of craftsmanship.
  - High Merchants above job level 39 whose pending advancement is MasterSmith are offered the
    job change once all their skill points are spent; accepting clears the pending advancement.
  - Everyone else is welcomed to Valhalla.

  ## Credits

  - Original from rAthena, authors and Contributors
    - Nana
    - Lupus
    - Vicious

  ## Adaptation

  - Transpiled by mix aesir.import.npcs from rAthena script
  - LLM-assisted Elixir refactor reviewed by Cλstor
  """

  use Aesir.ZoneServer.Npc,
    scope: :shared,
    spawn: [
      %{
        map: "valkyrie",
        x: 44,
        y: 50,
        dir: 5,
        sprite: 731,
        name: "MasterSmith",
        scope: :shared,
        unique_name: "MasterSmith#Valkyrie"
      }
    ]

  alias Aesir.ZoneServer.Script.Ctx
  alias Aesir.ZoneServer.Script.Rathena

  @impl true
  @spec on_talk(Ctx.t()) :: Ctx.t()
  def on_talk(ctx) do
    cond do
      get_char_var(ctx, :ADVJOB, 0) == 0 or upper(ctx) != 1 -> greet_non_candidate(ctx)
      eligible_for_advancement?(ctx) -> offer_advancement(ctx)
      true -> welcome_to_valhalla(ctx)
    end
  end

  defp eligible_for_advancement?(ctx) do
    get_char_var(ctx, :ADVJOB, 0) == Rathena.job_id(:whitesmith) and
      Rathena.job_id(class(ctx)) == Rathena.job_id(:merchant_high) and job_level(ctx) > 39
  end

  defp greet_non_candidate(ctx) do
    if Enum.random(1..10) > 4 do
      congratulate(ctx)
    else
      share_creed(ctx)
    end
  end

  defp congratulate(ctx) do
    ctx
    |> mes("[MasterSmith]")
    |> mes("Congratulations.")
    |> mes("Honor to the warriors!")
    |> close()
  end

  defp share_creed(ctx) do
    ctx
    |> mes("[MasterSmith]")
    |> mes("Ah...")
    |> mes("The pinnacle")
    |> mes("of craftsmanship.")
    |> mes("That's the work of")
    |> mes("a MasterSmith.")
    |> next()
    |> mes("[MasterSmith]")
    |> mes("Once you're the")
    |> mes("the best of the best,")
    |> mes("you begin forging with")
    |> mes("the goal of discovering")
    |> mes("new and better ways of")
    |> mes("crafting...")
    |> close()
  end

  defp offer_advancement(ctx) do
    {ctx, choice} =
      ctx
      |> mes("[MasterSmith]")
      |> mes("The time has come!")
      |> mes("Our world needs brave,")
      |> mes("hard-working adventurers")
      |> mes("like you...")
      |> next()
      |> mes("[MasterSmith]")
      |> mes("Would you like to")
      |> mes("become a MasterSmith?")
      |> next()
      |> select(["No.", "Yes."])

    cond do
      choice == 1 -> decline_advancement(ctx)
      Rathena.truthy?(skill_point(ctx)) -> request_skill_points_spent(ctx)
      true -> advance(ctx)
    end
  end

  defp decline_advancement(ctx) do
    ctx
    |> mes("[MasterSmith]")
    |> mes("When you're ready,")
    |> mes("feel free to come back.")
    |> mes("Honor to the warriors!")
    |> close()
  end

  defp request_skill_points_spent(ctx) do
    ctx
    |> mes("[MasterSmith]")
    |> mes("It is still possible for you to learn more skills. Please use")
    |> mes("all of your remaining Skill Points before returning to me.")
    |> close()
  end

  defp advance(ctx) do
    ctx
    |> jobchange(:whitesmith)
    |> set_char_var(:ADVJOB, 0)
    |> mes("[MasterSmith]")
    |> mes("Congratulations!")
    |> mes("As a MasterSmith,")
    |> mes("I hope you will forge")
    |> mes("a path towards a brighter")
    |> mes("future for Midgard.")
    |> close()
  end

  defp welcome_to_valhalla(ctx) do
    ctx
    |> mes("[MasterSmith]")
    |> mes("Welcome")
    |> mes("to Valhalla,")
    |> mes("the Hall of Honor.")
    |> next()
    |> mes("[MasterSmith]")
    |> mes("Please make")
    |> mes("yourself comfortable")
    |> mes("while you are here.")
    |> mes("Honor to the warriors!")
    |> close()
  end
end
