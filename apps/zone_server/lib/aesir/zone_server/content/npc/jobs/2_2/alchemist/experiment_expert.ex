defmodule Aesir.ZoneServer.Content.Npc.Jobs.M22.Alchemist.ExperimentExpert do
  @moduledoc """
  Van Helmont, a harried Alchemist researcher who quizzes candidates on Molgenstein's lessons.

  ## Behavior

  - Brushes off Alchemists and other non-Merchants while muttering about his experiments.
  - Sends candidates who ask to be taught to learn Counteragent and Mixture from Molgenstein.
  - Quizzes returning candidates on the ingredients, passing them on to Nicholas only when
    they actually completed Molgenstein's lesson and answer both questions correctly.
  ...
  defp talk_to_merchant(ctx, 8) do
    {ctx, choice} =
      ctx
      |> mes("Arrrrgh...!")
      |> mes(
    ...
    if choice == 2 do
      ...
      if request == 1 do
        send_to_molgenstein(ctx)
      else
        mutter_about_experiment(ctx)
      end
    end
  end

  defp talk_to_merchant(ctx, 9) do
    {ctx, choice} =
      ctx
      |> mes(
    ...
    if choice == 2 do
      ...
      if :erlang.band(get_char_var(ctx, :MISC_QUEST, 0), 4) != 0 do
        quiz_on_ingredients(ctx)
      else
        expose_unlearned_candidate(ctx)
      end
    end
  end

  defp talk_to_merchant(ctx, 20) do
    ctx
    |> mes("What...?")
  ...
  defp send_to_molgenstein(ctx) do
    ctx
    |> mes("[Van Helmont]")
    |> mes("Argh...!")
  ...
  defp mutter_about_experiment(ctx) do
    ctx
    |> mes("[Van Helmont]")
    |> mes("...")
    |> next()
  ...
  defp quiz_on_ingredients(ctx) do
    {ctx, counteragent_answer} =
      ctx
      |> mes("[Van Helmont]")
    ...
    if counteragent_answer != 1 or mixture_answer != 2 do
      ctx
      |> mes("[Van Helmont]")
  ...
  defp expose_unlearned_candidate(ctx) do
    {ctx, _counteragent_answer} =
      ctx
      |> mes("[Van Helmont]")
      |> mes("What item do")
