defmodule Aesir.ZoneServer.Content.Npc.Jobs.M21.Wizard.WhiteDog do
  @moduledoc """
  Maria, a Wizard turned into a talking dog, guides candidates through the Wizard job
  change quest.

  ## Behavior

  - Brushes off non-Mages and encourages existing Wizards to keep training.
  - Introduces herself to new candidates and optionally explains the three tests; the
    explanation and the "talking dog" retort send the player to Geffen Dungeon.
  - Gives hints on where to find the assigned magic items and encourages candidates at
    later quest stages.
  ...
  defp quest_stage(ctx, 0) do
    ...
    respond_to_explanation_offer(ctx, choice)
  end
  ...
  defp introduce_herself(ctx, 1) do
    ctx
    |> mes("[Dog]")
  ...
  defp introduce_herself(ctx, 2) do
    ctx
    |> cutin("job_wizard_maria02", 2)
    |> mes("[Dog]")
  ...
  defp introduce_herself(ctx, _choice), do: ctx

  defp respond_to_explanation_offer(ctx, 1) do
    ctx
    |> cutin("job_wizard_maria01", 2)
    |> mes("[Maria]")
  ...
  defp respond_to_explanation_offer(ctx, 2) do
    ctx
    |> mes("[Maria]")
  ...
  defp respond_to_explanation_offer(ctx, 3) do
    ctx
    |> cutin("job_wizard_maria02", 2)
    |> mes("[Maria]")
  ...
  defp respond_to_explanation_offer(ctx, _choice), do: ctx
end
