defmodule Aesir.ZoneServer.Content.Npc.Jobs.M22.Sage.PhysicsProfessor do
  @moduledoc """
  Aebecee George, the Sage academy professor who teaches elemental magic properties and
  guides the candidate's thesis.

  ## Behavior

  - Greets non-Mages with class-specific small talk.
  - Collects 30 Stones, hands out elemental ores, then collects 50 each of Crystal, Stone,
    and Wind Arrows and 1 Holy Water across two elemental lectures.
  - Consumes the thesis materials, walks the candidate through the thesis without grading
    the answers, grants the dissertation, and advances the quest.
  ...
  defp talk_to_candidate(ctx, 13), do: teach_class(ctx, get_char_var(ctx, :SAGE_Q2, 0))

  defp talk_to_candidate(ctx, 14) do
  ...
  defp teach_class(ctx, 1) do
    if count_item(ctx, 7049) > 29 do
      hand_out_elemental_ores(ctx)
    else
      ctx
  ...
  defp teach_class(ctx, 2) do
    if count_item(ctx, 1754) > 49 and count_item(ctx, 1756) > 49 and count_item(ctx, 1755) > 49 do
      lecture_on_strengths(ctx)
    else
      ctx
  ...
  defp teach_class(ctx, 3) do
    if count_item(ctx, 523) > 0 do
      lecture_on_weaknesses(ctx)
    else
      ctx
  ...
  defp teach_class(ctx, _step) do
    ctx |> mes("So...do you have the time? I'm kidding, girlfriend!") |> close()
  end

  defp hand_out_elemental_ores(ctx) do
    ctx
  ...
  defp lecture_on_strengths(ctx) do
    ctx
  ...
  defp lecture_on_weaknesses(ctx) do
    ctx
  ...
  defp write_chosen_line(ctx, options) do
    case select(ctx, options) do
      {ctx, choice} when choice in 1..3 -> mes(ctx, Enum.at(options, choice - 1))
      {ctx, _choice} -> ctx
    end
  end
end
