defmodule Aesir.ZoneServer.Content.Npc.PreRe.Jobs.Novice.Novice.Shion do
  @moduledoc """
  Shion welcomes novices to the pre-renewal training grounds.

  ## Behavior

  - Explains movement and directs novices to the training castle.
  - Grants introductory experience as the conversation progresses.
  - Returns advanced trainees to the training grounds.
  ...
  def on_talk(ctx) do
    if get_char_var(ctx, :nov_get_item04, 0) > 9 or get_char_var(ctx, :nov_get_item05, 0) > 9 do
      return_to_training(ctx)
    else
      greet(ctx, get_char_var(ctx, :nov_1st_cos, 0))
    end
  end

  defp return_to_training(ctx) do
    ctx
    |> mes("[Shion]")
    |> mes("Hm...?")
    |> mes("What are you")
  ...
  defp greet(ctx, progress) when progress > 2 do
    ctx
    |> mes("[Shion]")
  ...
  defp greet(ctx, _progress) do
    {ctx, choice} =
      ...
      |> next()
      |> select(["Where should I go?", "About Basic Interfaces.", "Who the crap are you?"])
