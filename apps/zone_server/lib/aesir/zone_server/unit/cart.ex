defmodule Aesir.ZoneServer.Unit.Cart do
  ...
  container-agnostic core in `Aesir.ZoneServer.Unit.Inventory` (which operates
  purely on a `%{index => InventoryItem.t()}` map, enforcing the same 100-slot
  cap and stacking rules), and the load path delegates to
  ...
  @doc """
  Adds `amount` of `item_def` to `cart`, reusing the inventory stacking core.

  Stacks, splits, and the 100-slot cap behave exactly as `Inventory.add/4`;
  weight is enforced by the orchestrator, not here.
  """
  @spec add(t(), ItemDefinition.t(), pos_integer(), map()) :: Inventory.op_result()
  defdelegate add(cart, item_def, amount, opts \\ %{}), to: Inventory

  @doc """
  Removes `amount` from the cart item at `index`, reusing the inventory core.
  """
  @spec remove(t(), non_neg_integer(), pos_integer()) :: Inventory.op_result()
  defdelegate remove(cart, index, amount), to: Inventory

  @doc """
  Total quantity of item `nameid` held across all stackable cart slots.
  """
  @spec held_amount(t(), integer()) :: non_neg_integer()
  defdelegate held_amount(cart, nameid), to: Inventory

  @doc """
  Index of a stackable cart slot holding item `nameid`, or `nil` when none.
  """
  @spec stackable_index(t(), integer()) :: non_neg_integer() | nil
  defdelegate stackable_index(cart, nameid), to: Inventory
end
