defmodule Aesir.ZoneServer.Mmo.Skills.Dancer.DcScream do
  ...
  @doc "Resolves a scheduled Dazzler event from the caster's live session state."
  @impl Active
  ...
  defp resolve_candidates(
    ...
    |> Enum.each(&apply_stun(&1, payload, caster, level))
  end
  ...
  defp apply_stun({unit_type, unit_id}, payload, caster, level) do
  ...
  defp enemy_chance(level), do: 25 + 5 * level
