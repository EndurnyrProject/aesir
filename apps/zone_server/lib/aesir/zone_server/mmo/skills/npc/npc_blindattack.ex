defmodule Aesir.ZoneServer.Mmo.Skills.Npc.NpcBlindattack do
  @moduledoc """
  Blind Attack (NPC_BLINDATTACK). Mob-only weapon strike that can blind.

  100% weapon damage, neutral element, no crit, cast at the mob's configured
  skill range. On a connecting hit it applies `sc_blind` with no explicit
  ...
  def cast(caster, {:unit, target_id}, level, definition),
    do: StatusStrike.cast(caster, target_id, level, definition, :sc_blind)
end
