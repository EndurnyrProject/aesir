defmodule Aesir.ZoneServer.Mmo.Skills.Mage.MgLightningbolt do
  @moduledoc """
  Lightning Bolt (MG_LIGHTNINGBOLT). Single-target wind magic that deals `level`
  separate magic hits at 100% MATK each.

  rAthena renewal: wind element, 100% ratio per hit, hit count equal to the cast
  level, range 9.
  """
  ...
  def cast(caster, {:unit, target_id}, level, _definition) do
    case MagicAttack.execute_bolt(caster, target_id, 20, level, []) do
      :ok -> {:ok, caster}
