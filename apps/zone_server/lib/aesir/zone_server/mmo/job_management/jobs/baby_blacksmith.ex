defmodule Aesir.ZoneServer.Mmo.JobManagement.Jobs.BabyBlacksmith do
  @moduledoc false
  use Aesir.ZoneServer.Mmo.JobManagement.Definition

  alias Aesir.ZoneServer.Mmo.JobManagement.Job
  alias Aesir.ZoneServer.Mmo.JobManagement.Tables

  @impl true
  def job do
    %Job{
      id: 4033,
      name: :baby_blacksmith,
      base_hp: Tables.BasepointsAlchemist.base_hp(),
      base_sp: Tables.BasepointsAlchemist.base_sp(),
      base_ap: Tables.BasepointsAlchemist.base_ap(),
      base_exp: Tables.ExpAlchemist.base_exp(),
      job_exp: Tables.ExpAlchemist.job_exp(),
      bonus_stats: %{
        1 => %Job.BonusStats{level: 1, dex: 1},
        2 => %Job.BonusStats{level: 1, dex: 1},
        3 => %Job.BonusStats{level: 3, str: 1},
        4 => %Job.BonusStats{level: 4, dex: 1},
        5 => %Job.BonusStats{level: 5, dex: 1},
        6 => %Job.BonusStats{level: 5, dex: 1},
        7 => %Job.BonusStats{level: 7, vit: 1},
        8 => %Job.BonusStats{level: 8, str: 1},
        9 => %Job.BonusStats{level: 9, dex: 1},
        10 => %Job.BonusStats{level: 9, dex: 1},
        11 => %Job.BonusStats{level: 11, luk: 1},
        12 => %Job.BonusStats{level: 12, dex: 1},
        13 => %Job.BonusStats{level: 13, vit: 1},
        14 => %Job.BonusStats{level: 13, vit: 1},
        15 => %Job.BonusStats{level: 13, vit: 1},
        16 => %Job.BonusStats{level: 16, str: 1},
        17 => %Job.BonusStats{level: 16, str: 1},
        18 => %Job.BonusStats{level: 16, str: 1},
        19 => %Job.BonusStats{level: 19, dex: 1},
        20 => %Job.BonusStats{level: 20, vit: 1},
        21 => %Job.BonusStats{level: 21, int: 1},
        22 => %Job.BonusStats{level: 21, int: 1},
        23 => %Job.BonusStats{level: 23, str: 1},
        24 => %Job.BonusStats{level: 23, str: 1},
        25 => %Job.BonusStats{level: 23, str: 1},
        26 => %Job.BonusStats{level: 26, dex: 1},
        27 => %Job.BonusStats{level: 26, dex: 1},
        28 => %Job.BonusStats{level: 28, dex: 1},
        29 => %Job.BonusStats{level: 29, agi: 1},
        30 => %Job.BonusStats{level: 29, agi: 1},
        31 => %Job.BonusStats{level: 31, str: 1},
        32 => %Job.BonusStats{level: 32, vit: 1},
        33 => %Job.BonusStats{level: 32, vit: 1},
        34 => %Job.BonusStats{level: 34, int: 1},
        35 => %Job.BonusStats{level: 34, int: 1},
        36 => %Job.BonusStats{level: 36, dex: 1},
        37 => %Job.BonusStats{level: 37, vit: 1},
        38 => %Job.BonusStats{level: 38, agi: 1},
        39 => %Job.BonusStats{level: 39, dex: 1},
        40 => %Job.BonusStats{level: 40, dex: 1},
        41 => %Job.BonusStats{level: 40, dex: 1},
        42 => %Job.BonusStats{level: 40, dex: 1},
        43 => %Job.BonusStats{level: 40, dex: 1},
        44 => %Job.BonusStats{level: 44, str: 1},
        45 => %Job.BonusStats{level: 44, str: 1},
        46 => %Job.BonusStats{level: 46, luk: 1},
        47 => %Job.BonusStats{level: 47, dex: 1},
        48 => %Job.BonusStats{level: 47, dex: 1},
        49 => %Job.BonusStats{level: 49, vit: 1},
        50 => %Job.BonusStats{level: 49, vit: 1}
      },
      base_aspd: %Job.BaseAspd{
        fist: 40,
        dagger: 50,
        one_handed_sword: 50,
        one_handed_axe: 46,
        two_handed_axe: 53,
        mace: 48,
        two_handed_mace: 53,
        shield: 5
      },
      hp_factor: 90,
      hp_increase: 0,
      sp_factor: 0,
      sp_increase: 400,
      ap_factor: 0,
      ap_increase: 0,
      max_weight: 30_000,
      max_base_level: 99,
      max_job_level: 50,
      max_stats: nil
    }
  end
end
