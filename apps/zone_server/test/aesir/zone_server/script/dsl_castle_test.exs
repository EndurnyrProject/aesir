defmodule Aesir.ZoneServer.Script.DslCastleTest do
  ...
  describe "castle_guardians/2" do
    test "returns eight entries in slot order, hired? true for the seeded slots" do
      castle = first_castle()
      :ok = CastleStore.hydrate(%{castle.id => row(1)})
      :ok = CastleStore.put_guardians(castle.id, [1, 4])

      result = Dsl.castle_guardians(build_ctx(), castle.id)

      assert Enum.map(result, & &1.slot) == Enum.to_list(0..7)
      assert Enum.map(result, & &1.type) == Enum.map(castle.guardians, & &1.type)
      assert Enum.map(result, & &1.hired?) == Enum.map(0..7, &(&1 in [1, 4]))
    end
  end

  describe "castle_guardian_hire_check/3" do
    test "raises on a detached ctx" do
      castle = first_castle()

      assert_raise ArgumentError, fn ->
        Dsl.castle_guardian_hire_check(detached_ctx(), castle.id, 0)
      end
    end
    ...
    test "returns :not_owner for a non-owner" do
      ...
      assert Dsl.castle_guardian_hire_check(ctx, castle.id, 0) == {:error, :not_owner}
    end
  end

  describe "castle_hire_guardian/3" do
    test "halts with :not_owner for a non-owner" do
      castle = first_castle()
      :ok = CastleStore.hydrate(%{castle.id => row(1)})
      ctx = build_ctx(guild_id: 2)

      result = Dsl.castle_hire_guardian(ctx, castle.id, 0)

      assert result.status == {:error, :not_owner}
    end

    test "leaves the ctx status ok and hires the slot for the owner" do
      castle = first_castle()
      :ok = CastleStore.hydrate(%{castle.id => row(1)})
      stub(GuildManager, :get, fn 1 -> {:ok, researched_guild(1)} end)
      ctx = build_ctx(guild_id: 1)

      result = Dsl.castle_hire_guardian(ctx, castle.id, 3)

      assert result.status == :ok
      assert 3 in CastleStore.guardians(castle.id)
    end
