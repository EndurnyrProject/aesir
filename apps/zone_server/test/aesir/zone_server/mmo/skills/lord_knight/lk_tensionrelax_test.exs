defmodule Aesir.ZoneServer.Mmo.Skills.LordKnight.LkTensionrelaxTest do
  ...
  test "a cast from the owning session applies the buff and sits after mailbox processing" do
    assert {:ok, definition} = Catalog.by_name(:lk_tensionrelax)
    assert definition.id == 358
    ...
    GenServer.cast(pid, {:skill, {:auto_cast, 358, 1, :self}})

    assert_eventually(fn ->
      StatusStorage.has_status?(:player, character.id, :sc_tensionrelax) and
        PlayerSession.get_state(pid).game_state.action_state == :sitting
    end)
  end
end
