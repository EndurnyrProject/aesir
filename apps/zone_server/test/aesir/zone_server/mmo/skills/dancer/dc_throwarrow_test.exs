defmodule Aesir.ZoneServer.Mmo.Skills.Dancer.DcThrowarrowTest do
  ...
  alias Aesir.ZoneServer.Mmo.Skills.Dancer.DcThrowarrow
  ...
  test "definition matches the pinned Throw Arrow table" do
    assert {:ok, DcThrowarrow} = Catalog.active_module_for(:dc_throwarrow)
    assert {:ok, definition} = Catalog.by_id(324)

    assert definition.name == :dc_throwarrow
    assert definition.display_name == "Throw Arrow"
  ...
  test "each level performs one aggregate ranged attack with two-hit presentation" do
    ...
    for level <- 1..5 do
      assert {:ok, ^caster} =
               DcThrowarrow.cast(caster, {:unit, @target_id}, level, definition)
    end
  end
  ...
  test "ordinary completion spends 12 SP and persists exactly one arrow after the attack succeeds" do
    ...
    assert {:ok, updated} =
             Interpreter.complete_cast(caster, 324, 3, {:unit, @target_id})
  ...
  test "range, weapon, and ammo gates are atomic and never execute the attack" do
    ...
    assert {:error, :wrong_weapon} = Interpreter.cast(wrong_weapon, 324, 3, {:unit, @target_id})
    assert_uncommitted(wrong_weapon)

    no_ammo = %{player_state() | inventory: %{}}
    stub_target_at(18, 10)
    assert {:error, :no_ammo} = Interpreter.cast(no_ammo, 324, 3, {:unit, @target_id})
    assert_uncommitted(no_ammo)

    out_of_range = player_state()
    stub_target_at(20, 10)
    assert {:error, :out_of_range} = Interpreter.cast(out_of_range, 324, 3, {:unit, @target_id})
  ...
  defp player_state do
    %PlayerState{
      ...
      stats: %Stats{
        ...
        progression: %PlayerProgression{learned_skills: %{324 => 5}},
        equipment: %Equipment{right_hand: @whip_id, ammo: @arrow_id}
      },
      inventory: %{
  ...
  test "classic carries the source's cast and SP" do
    definition = DcThrowarrow.definition()
