defmodule Aesir.ZoneServer.Mmo.Skills.Dancer.DcScreamTest do
  ...
  alias Aesir.ZoneServer.Mmo.Skills.Dancer.DcScream
  ...
  test "definition and cast capture the pinned delayed event" do
    ...
    assert {:ok, ^caster} =
             DcScream.cast(caster, :self, 3, definition, scheduler)

    assert_received {:scheduled, DcScream,
                     %{
                       map_name: "task20_map",
                       x: 100,
                       y: 200,
  ...
  test "enemy and current-party chances share the 4.5 second duration at every level" do
    ...
    stub(StatusInterpreter, :apply_status, fn unit_type, unit_id, :sc_stun, params ->
      send(test_pid, {:stun, unit_type, unit_id, params})
      :ok
    end)

    for {enemy_chance, party_chance, level} <-
          Enum.zip([[30, 35, 40, 45, 50], [7.5, 8.7, 10.0, 11.2, 12.5], 1..5]) do
      assert :ok = DcScream.deferred(payload(caster, level), caster)
  ...
  test "resolution uses the captured inclusive square and current eligibility" do
    ...
    {:ok, dead_party_member} =
      PlayerState.transition_to(player(8, x: 104, y: 200, party_id: 10), :dead)

    register(:player, dead_party_member)
    ...
    assert :ok = DcScream.deferred(payload, caster)
    ...
    refute_received {:stun, _, 1, _}
    refute_received {:stun, _, 3, _}
    refute_received {:stun, _, 5, _}
    refute_received {:stun, _, 6, _}
    refute_received {:stun, _, 7, _}
    refute_received {:stun, _, 8, _}
  end
  ...
  test "walking preserves the event and its captured origin" do
    ...
    expect(StatusInterpreter, :apply_status, fn :mob, 2, :sc_stun, _params ->
      send(test_pid, :stunned_at_original_origin)
      :ok
    end)

    assert :ok = DcScream.deferred(payload(original, 5), walked)
    assert_received :stunned_at_original_origin
  end
  ...
  test "death with revival and same-map or cross-map relocation invalidate the exact epoch" do
    ...
    for stale <- [revived, same_map_teleport, cross_map_teleport] do
      ...
      assert :ok = DcScream.deferred(payload(original, 5), stale)
  ...
  test "a caster removed from the spatial index cancels resolution" do
    ...
    assert :ok = DcScream.deferred(payload(caster, 5), caster)
  end
  ...
  test "disconnect destroys the process that owns the delayed event" do
    ...
    {pid, monitor} =
      spawn_monitor(fn ->
        ...
        assert {:ok, ^caster} =
                 DcScream.cast(caster, :self, 1, DcScream.definition(), scheduler)
    ...
    refute_receive {:skill, {:deferred, DcScream, _payload}}, 50
  end
