defmodule Aesir.ZoneServer.Mmo.Skills.Sage.SaSeismicweaponTest do
  ...
  alias Aesir.ZoneServer.Mmo.Skills.Sage.SaSeismicweapon
  ...
  describe "catalog registration" do
    test "by_id/1 resolves id 283" do
      assert {:ok, %{name: :sa_seismicweapon}} = Catalog.by_id(283)
    end

    test "by_name/1 resolves the atom" do
      assert {:ok, %{id: 283}} = Catalog.by_name(:sa_seismicweapon)
    end

    test "active_module_for/1 resolves the module" do
      assert {:ok, SaSeismicweapon} = Catalog.active_module_for(:sa_seismicweapon)
    end
  end
  ...
  describe "cast/4" do
    test "applies sc_earthweapon with val1=level and the tabulated duration" do
      ...
      expect(StatusInterpreter, :apply_status, fn :player, @caster_id, :sc_earthweapon, params ->
        assert params[:val1] == 4
        assert params[:caster_id] == @caster_id
        assert params[:duration] == 1_500_000
        :ok
      end)

      assert {:ok, ^caster} = SaSeismicweapon.cast(caster, :self, 4, definition)
    end
  end

  describe "validate/4 (bare-hand rejection)" do
    test "rejects a bare-handed self-cast" do
      assert {:error, :bare_handed} = SaSeismicweapon.validate(caster(), :self, 1, %{})
    end

    test "allows a self-cast target with a weapon equipped" do
      assert :ok = SaSeismicweapon.validate(caster(%Equipment{right_hand: 1201}), :self, 1, %{})
    end
  end
  ...
  describe "the validate-then-charge contract" do
    test "a bare-handed target fails before SP or the catalyst is spent" do
      ...
      gs =
        game_state(%{283 => 1}, %Equipment{}, %{
          0 => %InventoryItem{nameid: @catalyst_id, amount: 1, equip: 0}
        })

      assert {:error, :bare_handed} = SkillInterpreter.cast(gs, 283, 1, :self)
    end
  end
