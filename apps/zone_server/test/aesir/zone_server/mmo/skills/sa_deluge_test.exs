defmodule Aesir.ZoneServer.Mmo.Skills.SaDelugeTest do
  ...
  describe "on_place/1" do
    test "creates the tickless 7x7 element field shared by the trio" do
      assert {:ok, placement} = SaDeluge.on_place(group(level: 3))
  ...
  describe "field_support/1" do
    test "grants SC_DELUGE to every unit with no filter" do
      assert %{status_type: :sc_deluge, params: params, target?: target?} =
               SaDeluge.field_support(group(level: 4))

      assert params == [level: 4, val1: 4]
