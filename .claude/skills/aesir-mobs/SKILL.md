...
- Mob DB: `apps/zone_server/priv/db/mobs/mobs.yml` (`mix aesir.import.mobs`), loaded into
...
- **Trigger layer** — `mmo/mob_skill/` (`Db`, `Importer`, `Selector`, `CastingHandler`):
  the imported rows (`priv/db/mob_skills/`) own cast time, rate, state conditions, target
  codes.
